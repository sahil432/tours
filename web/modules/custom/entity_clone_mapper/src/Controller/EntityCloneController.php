...
namespace Drupal\entity_clone_mapper\Controller;
...
class EntityCloneController extends ControllerBase {
   ...
   *
   * @var \Drupal\entity_clone_mapper\Service\EntityCloneService
   */
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      ...
      $container->get('entity_clone_mapper.clone_service')
    );
  }
  ...
  public function cloneEntity($entity_type, EntityInterface $entity) {
    ...
    $config = $this->config('entity_clone_mapper.settings');
    ...
    if (count($available_mappings) === 1) {
      ...
      if ($entity->isTranslatable() && count($entity->getTranslationLanguages()) > 1) {
        return $this->formBuilder()->getForm(
          'Drupal\entity_clone_mapper\Form\CloneLanguageSelectionForm',
          $entity,
          $mapping
        );
      }
    ...
    return $this->formBuilder()->getForm(
      'Drupal\entity_clone_mapper\Form\CloneSelectionForm',
      $entity,
      $available_mappings
    );
  }
