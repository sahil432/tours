...
- **Clone Button**: Adds a "Clone to Another Type" button on entity edit forms
...
1. Copy the entire `entity_clone_mapper` directory to your Drupal installation's `modules/custom/` directory.

2. Enable the module:
   ```bash
   drush en entity_clone_mapper
   ```
   
   Or via the UI: Admin > Extend, find "Entity Clone Mapper" and enable it.
...
1. Navigate to **Admin > Configuration > Content > Entity Clone Mapper** (`/admin/config/content/entity-clone-mapper`)
...
1. **Administer Entity Clone Mapper**: Access to configuration form
...
3. Register your service in `entity_clone_mapper.services.yml`

## Module Files Structure

```
entity_clone_mapper/
├── entity_clone_mapper.info.yml
├── entity_clone_mapper.module
├── entity_clone_mapper.routing.yml
├── entity_clone_mapper.permissions.yml
├── entity_clone_mapper.links.menu.yml
├── entity_clone_mapper.services.yml
├── README.md
└── src/
    ├── Controller/
